...
fon = transform.scale(image.load("11.jpg"),(win_width,win_height))
...
rock = sprite.Group()
rocket = rocket("111.png" , x1, y1, 8)
rock.add(rocket)
asteroid = sprite.Group()
for i in range(2):
    ast = Enemy('femka.png',randint(0,800), 1, 1)
    asteroid.add(ast)

monsters = sprite.Group()
for i in range (6):
    ufo = Enemy('nigga.png', randint(0,800), 1, 1)
    monsters.add(ufo)
...
while game:
    ...
    if finish != True:
        ...
        if len(monsters) < 6:
            ufo = Enemy('nigga.png', randint(0,750), 1, 1)
            monsters.add(ufo)

        if len(asteroid) < 1:
            ast = Enemy('femka.png', randint(0,750), 1, 2)
            asteroid.add(ast)
